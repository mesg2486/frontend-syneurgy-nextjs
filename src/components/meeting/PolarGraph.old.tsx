...
const PolarGraph = ({
  ...
  console.log({ distance, avgSync, wordCount, emoji, groupEmoji, groupSync });
  ...
  useEffect(() => {
    ...
    userKeys.forEach((userKey, index) => {
      ...
      if (!timeData) return;
      // TODO
      const distance2 = timeData[1];

      let currentRadius = 50 + (distance2 / 1.5) * (150 - 50);
      const currentAngle = anglePerUser * index;
      ...
      const userEmotions = emoji[userKey];
      let emojiId = 0;
