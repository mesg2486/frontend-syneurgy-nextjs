...
// import emotion1Url from "../../assets/img/emotions/emotion1.png";
// import emotion2Url from "../../assets/img/emotions/emotion2.png";
// // ... import other emotion URLs
...
//     Object.keys(wordCount).forEach((userKey) => {
//       const userData = wordCount[userKey];
//       if (!userData) return;
//       userData.forEach(([time, count]) => {
//         if (time === closestTime) {
...
//       userData.forEach(([time, count]) => {
//         if (time === closestTime) {
//           const percentage = Math.round((count / totalWordCount) * 100);
//           wordCountPercentage.push([
//             userKey.replace("speaker", "user"),
//             percentage,
//           ]);
//         }
//       });
...
//     pulse,
//   ) => {
//     let timeInMilliseconds = currentTime * 1000;
//     let timeIndex = userData.findIndex(
//       (data) => data[0] === timeInMilliseconds,
//     );
...
//     let dotX =
//       width / 2 + currentRadius * Math.cos(currentAngle) - initialRadius / 2; // Adjust the position
//     let dotY =
//       height / 2 + currentRadius * Math.sin(currentAngle) - initialRadius / 2; // Adjust the position
...
//         const xDirection = xCenter - bbox.x; // calculate direction to center (x)
//         const yDirection = yCenter - bbox.y; // calculate direction to center (y)
//         const randomX = xDirection * 0.05; // add some randomness to the direction
//         const randomY = yDirection * 0.05; // add some randomness to the direction
//         return `translate(${randomX}, ${randomY})`; // translate towards the center with some randomness
//       })
//       .transition()
//       .duration(1000)
//       .ease(d3.easeSinIn)
//       .style("opacity", initialOpacity)
//       .attr("transform", "translate(0, 0)") // Reset translation
//       .on("end", () => pulse(selection, initialOpacity));
//   };

//   const updateSyncMeter = (closestTime) => {
//     const index = groupSync.findIndex((obj) => obj[0] === closestTime * 1000);
//     if (index === -1) return;
//     let current = groupSync[index][1];
//     let acc_average = groupSync[index][2];
...
//     d3.select("#averageSyncIcon")
//       .transition()
//       .duration(500)
//       .ease(d3.easeSinIn)
//       .attr("x", (width / 4) * 3 + 14 + current * 100);

//     d3.select("#averageTeamSyncIcon")
//       .transition()
//       .duration(500)
//       .ease(d3.easeSinIn)
//       .attr("x", (width / 4) * 3 + 14 + acc_average * 100);
//   };

//   const updateEmojiMeters = (closestTime) => {
//     const key = "timeline";
//     const index = groupEmoji.findIndex(
//       (obj) => obj.hasOwnProperty(key) && obj[key] === closestTime * 1000,
//     );
//     if (index === -1) return;
//     let current = groupEmoji[index].current;
//     let acc_average = groupEmoji[index].acc_average;

//     emotions.forEach((emotion) => {
//       if (emotion.id === current) {
//         d3.select("#currentEmoji").attr("href", emotion.url);
//       }
//       if (emotion.id === acc_average) {
//         d3.select("#averageEmoji").attr("href", emotion.url);
//       }
//     });
//   };
...
//       "#727F9F",
//     ];
//     let originalText = ["Zone 1", "Zone 2", "Zone 3", "Zone 4", "Zone 5", ""];

//     if (dotsPosition[0] === undefined) return;
//     let dotsInCircle = [0, 0, 0, 0, 0];
//     dotsPosition.forEach((dot) => {
//       if (dot < 70) {
//         dotsInCircle[4] = dotsInCircle[4] ? dotsInCircle[4] + 1 : 1;
//       } else if (dot < 90) {
//         dotsInCircle[3] = dotsInCircle[3] ? dotsInCircle[3] + 1 : 1;
//       } else if (dot < 110) {
//         dotsInCircle[2] = dotsInCircle[2] ? dotsInCircle[2] + 1 : 1;
//       } else if (dot < 130) {
//         dotsInCircle[1] = dotsInCircle[1] ? dotsInCircle[1] + 1 : 1;
//       } else if (dot <= 150) {
//         dotsInCircle[0] = dotsInCircle[0] ? dotsInCircle[0] + 1 : 1;
//       }
//     });

//     let highestNumberIndex = dotsInCircle.reduce(
//       (maxIndex, val, currentIndex, arr) => {
//         return val !== null && (arr[maxIndex] === null || val > arr[maxIndex])
//           ? currentIndex
//           : maxIndex;
//       },
//       0,
...
//     if (previousHighestNumberIndex >= -1) {
//       d3.select(`#circle_${previousHighestNumberIndex}`)
//         .transition()
//         .duration(500)
//         .attr("fill", originalColors[previousHighestNumberIndex])
//         .attr("stroke", originalStroke[previousHighestNumberIndex]);

//       d3.select(`#line_${previousHighestNumberIndex}`)
//         .transition()
//         .duration(500)
//         .attr("stroke", originalStroke[previousHighestNumberIndex]);

//       d3.select(`#text_${previousHighestNumberIndex}`)
//         .transition()
//         .duration(500)
//         .attr("fill", "#fff");
//     }

//     d3.select(`#circle_${highestNumberIndex}`)
//       .transition()
//       .duration(500)
//       .attr("fill", "#276B90")
//       .attr("stroke", "#58C4FF");

//     d3.select(`#line_${highestNumberIndex}`)
//       .transition()
//       .duration(500)
//       .attr("stroke", "#58C4FF");

//     d3.select(`#text_${highestNumberIndex}`)
//       .transition()
//       .duration(500)
//       .attr("fill", "#58C4FF");
//     d3.select("#text_0").text(dotsInCircle[0] + "   " + originalText[0]);
//     d3.select("#text_1").text(dotsInCircle[1] + "   " + originalText[1]);
//     d3.select("#text_2").text(dotsInCircle[2] + "   " + originalText[2]);
//     d3.select("#text_3").text(dotsInCircle[3] + "   " + originalText[3]);
//     d3.select("#text_4").text(dotsInCircle[4] + "   " + originalText[4]);
//     setPreviousHighestNumberIndex(highestNumberIndex);
//   };

//   const handleSeekToSecond = (currentTime) => {
//     const wordCountPercentage = getWordPercentage(currentTime);

//     userKeys.forEach((userKey, index) => {
//       let userData = distance[userKey];
//       if (!userData) return;
//       let closestTime = Math.floor(currentTime / 30) * 30 + 1;
...
//       let emotion =
//         emotions.find((emotion) => emotion.id === emojiId)?.url ||
//         emotions[0].url;

//       let currentRadius = updateDotPosition(
//         userData,
//         index,
//         closestTime,
//         initialRadius,
...
//     });
//     updateSyncMeter(Math.floor(currentTime / 30) * 30 + 1);
//     updateEmojiMeters(Math.floor(currentTime / 30) * 30 + 1);
//     setTimeout(() => {
//       updateCircleColor();
//     }, 1000);
//   };
...
//         const radius = calculateRadius(wordCountPercentage, userKey);
//         const emotion = getEmotion(emoji[userKey], closestTime);
...
//       updateSyncMeter(closestTime);
//       updateEmojiMeters(closestTime);
//       updateCircleColor();
//       time++;
//     }, 1000);
...
//         let titleWidth = svg.select("#title_chart").node().getBBox().width;
//         svg
...
//         .on("mousemove", (event) => {
//           tooltip
//             .style("left", `${(event.pageX * 1) / 2}px`)
//             .style("top", `${(event.pageY * 1) / 2}px`)
//             .style("opacity", 1);
//         })
//         .on("mouseleave", () => {
...
//   return <div ref={chartRef} className="w-full absolute bg-black-200"></div>;
// };
